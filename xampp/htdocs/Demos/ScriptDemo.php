...
// "Retrieve Desired Record Set"
	...
	//$strCategory = "Tools";
	$strSQL = "SELECT ID, Category, Description, Image, Price, Option1Desc, Option1a, Option1b,
    Option1c, Option1d, Option2Desc, Option2a, Option2b, Option2c, Option2d FROM Products ORDER BY ID";
  ...
  while ($rowProd = mysqli_fetch_array($rsProd)) {
	...
	//	  Keep that in mind for future exercises.
	   ...
	   echo '
		...
		//I have no idea how else you are going to show these. But this is how I was able to get these to display
		//This is to filter the outputs so that they only show the ones that have information inside them
		//Sprint 2 exercise
		...
		if (!empty($rowProd["Option1Desc"])) {
			echo '
				...
				<ul> ';
				if (!empty($rowProd["Option1a"])) {
					echo '<li>' . $rowProd["Option1a"] . '</li>';
				}
				if (!empty($rowProd["Option1b"])) {
					echo '<li>' . $rowProd["Option1b"] . '</li>';
				}
				if (!empty($rowProd["Option1c"])) {
					echo '<li>' . $rowProd["Option1c"] . '</li>';
				}
				if (!empty($rowProd["Option1d"])) {
		...
		if (!empty($rowProd["Option2Desc"])) {
			echo '
				...
				<ul> ';
				if (!empty($rowProd["Option2a"])) {
					echo '<li>' . $rowProd["Option2a"] . '</li>';
				}
				if (!empty($rowProd["Option2b"])) {
					echo '<li>' . $rowProd["Option2b"] . '</li>';
				}
				if (!empty($rowProd["Option2c"])) {
					echo '<li>' . $rowProd["Option2c"] . '</li>';
				}
				if (!empty($rowProd["Option2d"])) {
